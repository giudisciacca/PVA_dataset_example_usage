...
dataset = load_dataset(path=folder + file + '.h5')
dataset += load_dataset(path=folder + file + '_v_t.h5')
dataset += load_dataset(path=folder + file + '_t.h5')
n_total = len(dataset)
n_val = n_total // 10
n_test = n_total // 10
...
for model in [ DiffNet(in_channels=1),UNet(n_channels=1, n_classes=1) ]:
    ...
    if TRAIN == True:
        print(f"Training model: {model._get_name()}")
    ...
    print(f"Evaluating model: {model._get_name()}")
    model.load_state_dict(torch.load(model._get_name()+"_best_model.pt"))
    model.to(device)
    test_loss = evaluate(test_loader)
    print(f"Test Loss: {test_loss:.4f}")

    print(f"Evaluating model {model._get_name()} on test dataset")
    model.eval()
    to_pil = T.ToPILImage()
    n_grid = 17
    input_images = []
    ...
    # test on checkerboard
    control_dataset = load_dataset(path=folder + file + '.h5',  input_data_string='imagesControl', target_data_string='imagesInputControl')
    control_dataset += load_dataset(path=folder + file + '_v_t.h5', input_data_string='imagesControl', target_data_string='imagesInputControl')
    control_dataset += load_dataset(path=folder + file + '_t.h5',   input_data_string='imagesControl', target_data_string='imagesInputControl')
    n_total = len(control_dataset)
    n_val = n_total // 10
    n_test = n_total // 10
    n_train = n_total - n_val - n_test

    _, _, ctest_set = random_split(control_dataset, [n_train, n_val, n_test])
    ctest_loader = DataLoader(ctest_set, batch_size=batch_size)

    print(f"Testing model: {model._get_name()} on checkerboard dataset")
    ...
    save_dir = os.path.dirname(f"{file}_{model._get_name()}_input_grid_control.png")
    if save_dir and not os.path.exists(save_dir):
        os.makedirs(save_dir, exist_ok=True)

    to_pil(input_grid).save(f"{file}_{model._get_name()}_input_grid_control.png")
    to_pil(output_grid).save(f"{file}_{model._get_name()}_output_grid_control.png")
    to_pil(target_grid).save(f"{file}_{model._get_name()}_target_grid_control.png")

print("End of run_training.py")
